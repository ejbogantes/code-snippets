...
// creates a document
export const getDocumentData = async (params: object, headers: object = {}) => {
  // gets the token
  const token = await getToken('GET:management/getDocument');
  ...
  const res = await DocumentsApi.get(`management/getDocument`, { params });
  return res.data;
};
