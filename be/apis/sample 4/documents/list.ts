...
export const listDocuments = async (params: object, headers: object = {}) => {
  // gets the token
  const token = await getToken('GET:management/listDocuments');
  ...
  const res = await DocumentsApi.get(`management/listDocuments`, { params });
  return res.data;
};
