...
/**
 * @swagger
 * /api/soom-website/notifications/partner-request:
 *  post:
 *    description: Send a partner request form notifications
 *    operationId: partnerRequestFormNotification
 *    tags:
 *      - Notifications
 *    security:
 *      - apiKey: []
 ...
 *          application/json:
 *            schema:
 *              $ref: '#/definitions/responses/notifications/partnerRequest'
 *      400:
  ...
  switch (method) {
    case 'POST':
      try {
        ...
        // render the email
        const htmlAdmin = await getTemplate('soom-website-partner-request-admin', {
          company,
          name,
          phone,
          email,
        ...
        const emailAdmin = await sendEmail({
          ...
          HtmlPart: htmlAdmin,
          TextPart: '',
          SubjectPart: `Soom - New Partner Request`,
          Source: fromEmail
        });
        if (!emailAdmin) {
        ...
        if (email) {
          ...
          try {
            // render the email
            const htmlUser = await getTemplate('soom-website-partner-request-user', {
              company,
              name,
              phone,
              email,
            ...
            await sendEmail({
              ...
              HtmlPart: htmlUser,
              TextPart: '',
              SubjectPart: `Soom - Partner Request Received`,
              Source: fromEmail
            });
          } catch (e) {
            console.error(e);
