...
/**
 * @swagger
 * /api/soom-website/notifications/demo-request:
 *  post:
 *    description: Send a demo request form notifications
 *    operationId: demoRequestFormNotification
 *    tags:
 *      - Notifications
 *    security:
 *      - apiKey: []
 ...
 *          application/json:
 *            schema:
 *              $ref: '#/definitions/responses/notifications/demoRequest'
 *      400:
...
const handler = async (request, response) => {
  ...
  switch (method) {
    case 'POST':
      try {
        if (!body.company || !body.name || !body.phone || !body.email || !body.product || !body.message) {
          return response.status(400).json({
            ...
            params: {
              ...
              product: !body.product ? `Product is required` : undefined,
              message: !body.message ? `A message is required` : undefined
            }
          });
        }

        const { company, name, phone, email, product, message } = body;
        ...
        // render the email
        const htmlAdmin = await getTemplate('soom-website-demo-request-admin', {
          company,
          name,
          phone,
          email,
        ...
        const emailAdmin = await sendEmail({
          ...
          HtmlPart: htmlAdmin,
          TextPart: '',
          SubjectPart: `Soom - New Demo Request`,
          Source: fromEmail
        });
        if (!emailAdmin) {
        ...
        if (email) {
          ...
          try {
            // render the email
            const htmlUser = await getTemplate('soom-website-demo-request-user', {
              company,
              name,
              phone,
              email,
            ...
            await sendEmail({
              ...
              HtmlPart: htmlUser,
              TextPart: '',
              SubjectPart: `Soom - Demo Request Received`,
              Source: fromEmail
            });
          } catch (e) {
            console.error(e);
