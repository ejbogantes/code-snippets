import { PDFDocument, PDFName, PDFNumber, PDFHexString, PDFString } from 'pdf-lib';
import signer from 'node-signpdf';

import PDFArrayCustom from './PDFArrayCustom.class';

export default class SignPDF {
  constructor(pdfBuffer, certBuffer, certPassword = '', signReason = '') {
  ...
  async signPDF() {
    ...
    newPDF = signer.sign(newPDF, this.certificate, { passphrase: this.certificatePassword });
  ...
  async _addPlaceholder() {
    ...
    const SIGNATURE_LENGTH = 4750; // 3322;
    ...
    const signatureDict = loadedPdf.context.obj({
      ...
      Reason: PDFString.of(this.signReason),
